...
TEST_CASE( "Grover algorithms") {
    ...
    SECTION(N_2_A_01){
        ...
        for(size_t i = 0; i < MINIMAL_TEST_AMOUNT; i++){
            ...
            REQUIRE((bitset[0] == 0 && bitset[1] == 0 && bitset[2] == 1 && bitset[3] == 0 && bitset[4] == 0));
        }
    }
    
    constexpr char N_2_A_10[] ="N=2 A=10";
    SECTION(N_2_A_10){
        DebugQuantum5 emulator;
        auto ent = emulator.generateRegister();
        emulator.hadamardGate(1, ent);
        ...
        for(size_t i = 0; i < MINIMAL_TEST_AMOUNT; i++){
            ...
            INFO(N_2_A_10 <<" failed because answer that has been given: " << bitset << " is wrong");
            REQUIRE((bitset[4] == 0 && bitset[3] == 1 && bitset[2] == 0 && bitset[1] == 0 && bitset[0] == 0));
            } catch(const QuantumException& exp){
                 FAIL(N_2_A_10 << " failed with the exception " << exp.getMessage() );
            }
